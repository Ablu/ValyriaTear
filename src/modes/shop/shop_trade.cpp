...
void TradeInterface::_ChangeViewMode(SHOP_VIEW_MODE new_mode)
{
    ...
    if(new_mode == SHOP_VIEW_MODE_LIST) {
        ...
        //this-> _selected_properties.SetOptionText(0, MakeUnicodeString(NumberToString(_selected_object->GetBuyPrice())));
        //is replaced with below
        /*for(uint32 i = 0; i < _selected_object->GetTradePrice()->size(); i++){
            GlobalObject *new_object = GlobalCreateNewObject((*(_selected_object->GetTradePrice()))[i], 1);
            if(new_object != NULL) {
                ShopObject *new_shop_object = new ShopObject(new_object);
                _selected_properties.AddOption(MakeUnicodeString("<" + new_shop_object->GetObject()->GetIconImage().GetFilename() + "><30>")
                    + (*(new_shop_object->GetTradePrice()))[i].GetObject()->GetName());
            }
        }*/
        //is replaced with above
...
void TradeListDisplay::ReconstructList()
{
    ...
    for(uint32 i = 0; i < _objects.size(); i++) {
        ...
        /*for(uint32 j = 0; j < obj->GetTradePrice()->size(); j++){
            GlobalObject *new_object = GlobalCreateNewObject((*(obj->GetTradePrice()))[i], 1);
            if(new_object != NULL) {
                ShopObject *new_shop_object = new ShopObject(new_object);
                _property_list.AddOption(MakeUnicodeString("<" + (*(obj->GetTradePrice()))[j]->GetObject()->GetIconImage().GetFilename() + "><30>")
                    + (*(obj->GetTradePrice()))[j]->GetObject()->GetName());
            }
        }*/
...
bool TradeListDisplay::ChangeTradeQuantity(bool less_or_more, uint32 amount)
{
    ...
    if(less_or_more == false) {
        ...
        //ShopMode::CurrentInstance()->UpdateFinances(0, 0);
